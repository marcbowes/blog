...
don't have to an instance type (do I want large or 4xlarge?). A service where
there is no infrastructure to manage, or patch. A service that is pay-as-you-go,
that scales to meet your needs. Then, add in the capabilities of a relational
database. The ability to run complex transactions, do joins, and evolve your
schema over time.
